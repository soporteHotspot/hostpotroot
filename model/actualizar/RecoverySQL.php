...
$obtenermoneda=("SELECT * FROM sistema");        
               $moneda=mysqli_query($mysqli,$obtenermoneda);    
               $ver=mysqli_fetch_array($moneda);
          

               
                if (isset($ver["zona"])) {$zonaempresa=$ver["zona"];  
...
define("BACKUP_FILE", $_POST["nombre"]); // Script will autodetect if backup file is gzipped based on .gz extension
define("CHARSET", 'utf8');
...
set_time_limit(900); // 15 minutes
    ...
    require("../../require/connect_db.php");
  $checkcode=mysqli_query($mysqli,"SELECT * FROM recoverydb WHERE code='$codigo' AND email='$emailadmin'");
...
{
$restoreDatabase = new Restore_Database($server,$usernamebase,$passwordbase, $database);
$result = $restoreDatabase->restoreDb(BACKUP_DIR, BACKUP_FILE); 

  mysqli_query($mysqli,"INSERT INTO `eventos`(`id`, `hora`, `fecha`, `evento`, `tipo`) VALUES('', '$hora', '$fecha', '$username restauró con fichero  $fichero ', 'success')");
$sql = "DELETE  FROM recoverydb  WHERE email='$emailadmin' ";
