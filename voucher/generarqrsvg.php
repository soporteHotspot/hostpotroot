...
class QRCode {
    ...
    function printHTML($size = "2px") {
        $nombre=$nombre;
$file = fopen("temp/'$nombre'.htm", "w");
        $style = "border-style:none;border-collapse:collapse;margin:0px;padding:0px;";

        fwrite($file,"<table style='$style'>". PHP_EOL);

        for ($r = 0; $r < $this->getModuleCount(); $r++) {
$file = fopen("temp/qr.htm", "w");
            fwrite($file,"<tr style='$style'>". PHP_EOL);

            for ($c = 0; $c < $this->getModuleCount(); $c++) {
                $color = $this->isDark($r, $c)? "#000000" : "#ffffff";
                fwrite($file,"<td style='$style;width:$size;height:$size;background-color:$color'></td>". PHP_EOL);
            }
    ...
    public function printSVG($carpetatem,$nombre,$size)
    {   


        
if (!file_exists($carpetatem)) {
    mkdir($carpetatem, 0777, true);
    
    }
    

         $file = fopen($carpetatem.$nombre.".svg", "w");
        $width = $this->getModuleCount() * $size;
        $height = $width;
        fwrite($file, '<svg   width="' . $width . '" height="' . $height . '" viewBox="0 0 ' . $width . ' ' . $height . '" xmlns="http://www.w3.org/2000/svg">'. PHP_EOL);

        for ($r = 0; $r < $this->getModuleCount(); $r++) {
            for ($c = 0; $c < $this->getModuleCount(); $c++) {
                $color = $this->isDark($r, $c) ? "#000000" : "#ffffff";
                fwrite($file, '<rect x="' . ($c * $size) . '" y="' . ($r * $size) . '" width="' . $size . '" height="' . $size . '" fill="' . $color . '" shape-rendering="crispEdges"/>'. PHP_EOL);
            }
        }

        fwrite($file, '</svg>'. PHP_EOL);
        fclose($file);
    }
}
